...
import { MessageCircle, Link as LinkIcon, ShieldCheck, Printer } from "lucide-react";
import { DAMAGE_TYPES } from "@/lib/inspection-areas";
import { toast } from "sonner";

const damageLabel = (v: string) => DAMAGE_TYPES.find((d) => d.value === v)?.label ?? v;

export function ReportView({ inspection, signature, publicMode = false }: { inspection: any; signature: any; publicMode?: boolean }) {
    ...
    <div className="space-y-4">
      {!publicMode && (
        <div className="flex flex-wrap gap-2">
          <Button onClick={sendWhatsApp} className="bg-success text-success-foreground hover:bg-success/90"><MessageCircle className="mr-2 h-4 w-4" /> Enviar por WhatsApp</Button>
          <Button variant="outline" onClick={() => { navigator.clipboard.writeText(link); toast.success("Link copiado"); }}><LinkIcon className="mr-2 h-4 w-4" /> Copiar link público</Button>
      ...
      <article className="space-y-6 rounded-2xl border bg-card p-6 shadow-card print:border-0 print:shadow-none">
        <header className="flex items-start justify-between border-b pb-4">
          <div>
            ...
            <h1 className="mt-2 font-display text-2xl font-bold">Laudo de Vistoria Digital</h1>
            <div className="font-mono text-xs text-muted-foreground">Código: {inspection.unique_code}</div>
          </div>
          <div className="text-right text-xs text-muted-foreground">
            <div>Entrada: {formatDateTime(inspection.entry_datetime)}</div>
            {inspection.finalized_at && <div>Finalizada: {formatDateTime(inspection.finalized_at)}</div>}
          </div>
        </header>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <Section title="Veículo">
            ...
            <Field label="Marca/Modelo" value={`${v?.brand ?? ""} ${v?.model ?? ""}`.trim()} />
            <Field label="Ano/Cor" value={`${v?.year ?? "—"} • ${v?.color ?? "—"}`} />
            <Field label="Quilometragem" value={v?.mileage ? `${v.mileage} km` : "—"} />
            <Field label="Serviço" value={inspection.service_type} />
          </Section>
        </div>

        <section>
          <h2 className="mb-3 font-display text-lg font-semibold">Resumo dos danos</h2>
          {damages.length === 0 ? (
            <div className="rounded-lg bg-success/10 p-3 text-sm text-success">Nenhum dano registrado na entrada.</div>
          ) : (
            <div className="overflow-hidden rounded-lg border">
              <table className="w-full text-sm">
                <thead className="bg-muted/50 text-left text-xs uppercase text-muted-foreground">
                  <tr><th className="px-3 py-2">Área</th><th className="px-3 py-2">Tipo</th><th className="px-3 py-2">Observação</th></tr>
                </thead>
                <tbody>
                  {damages.map((d) => {
                    const area = areas.find((a) => a.id === d.inspection_area_id);
                    return (
                      <tr key={d.id} className="border-t">
                        <td className="px-3 py-2">{area?.area_name ?? "—"}</td>
                        <td className="px-3 py-2"><span className="rounded bg-destructive/10 px-2 py-0.5 text-xs text-destructive">{damageLabel(d.damage_type)}</span></td>
        ...
        {photos.length > 0 && (
          <section>
            <h2 className="mb-3 font-display text-lg font-semibold">Fotos da vistoria</h2>
              ...
              {photos.map((p) => {
                ...
                return (
                  <figure key={p.id} className="overflow-hidden rounded-lg border bg-muted">
                    <img src={p.original_url} alt={area?.area_name ?? ""} className="aspect-square w-full object-cover" />
                    <figcaption className="px-2 py-1 text-[11px] text-muted-foreground">{area?.area_name ?? "—"}</figcaption>
                  </figure>
                );
              })}
            </div>
